...
public class RemoteFieldChangeListener implements WatcherListener{
	
	private Field watchField;
	private FieldChangeCallback callback;
	
	public RemoteFieldChangeListener(Field watchField,FieldChangeCallback callback){
		this.watchField = watchField;
		this.callback = callback;
	}
	
	public void onDelete(Watcher w, String key) {
		callback.onDelete(key,watchField,DataFrom.REMOTE);
	}

	public void onAdd(Watcher w, String key) {
		callback.onAdd(key,watchField,DataFrom.REMOTE);
	}

	public void onUpdate(Watcher w, String key) {
		callback.onUpdate(key,watchField,DataFrom.REMOTE);
	}
}